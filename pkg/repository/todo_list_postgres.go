...
func (r *TodoListPostgres) Create(userId int, list todo.TodoList) (int, error) {
	// Запрос выполняем в виде транзакции (либо все выполнится, либо откатываемся назад)
	tx, err := r.db.Begin()
	if err != nil {
		return 0, err
	}
	...
	if err := row.Scan(&id); err != nil {
		tx.Rollback() // откатываемся назад в случае ошибки
		return 0, err
	}
...
func (r *TodoListPostgres) GetListById(userId, listId int) (todo.TodoList, error) {
	...
	query := fmt.Sprintf("SELECT tl.id, tl.title, tl.description FROM %s tl INNER JOIN %s ul on tl.id = ul.list_id WHERE ul.id = $1 AND ul.list_id = $2",
		todoListsTable, usersListTable)
...
func (r *TodoListPostgres) Update(userId, listId int, updatedList todo.UpdateListInput) error {
	setValues := make([]string, 0)
	args := make([]interface{}, 0)
	argId := 1

	if updatedList.Title != nil {
		setValues = append(setValues, fmt.Sprintf("title=$%d", argId))
		args = append(args, *updatedList.Title)
		argId++
	}

	if updatedList.Description != nil {
		setValues = append(setValues, fmt.Sprintf("description=$%d", argId))
		args = append(args, *updatedList.Description)
		argId++
	}
